...
# - Use the `@intervention` macro to apply parametric interventions (e.g., simulating a drug that scales receptor conductance) without rebuilding the full model.
...
@graph g begin
    @nodes begin
        ne = HHExciNeuron(; I_bg=8)
        ni = HHInhiNeuron(; I_bg=0.4)
    end
    @connections begin
...
@graph g begin
    @nodes begin
        ...
        Layer_2_3 = Cortical(N_wta=20, N_exci=5, density=0.05, weight=1, I_bg=4, G_syn_inhi=4, τ_inhi=70, G_syn_ff_inhi=1.5)
    end
    @connections begin
...
prob_EI = @intervention prob begin
    @setup begin
        p_I = 1.5 ## GABA factor
        p_E = 3.0 ## AMPA factor
    end
...
# After solving the circuit model in its control condition, we design an `@intervention` above to simulate an intervention on both the GABA A and the AMPA receptors, which are the default receptos in the `Cortical` blox. This way we change the E-I balance of the circuit by affecting both components.
