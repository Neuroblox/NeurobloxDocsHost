...
# In the previous tutorials (see [Getting Started](@ref getting_started_julia) and the PING tutorial),
# we modeled brain circuits by explicitly simulating individual neurons — each with its own membrane voltage
# and ionic currents. This approach is biologically detailed but becomes computationally expensive as the
# number of neurons grows.
#
# In this tutorial we zoom out: instead of tracking every neuron individually, we use **neural mass models**
# that describe the *average* activity of an entire population of neurons with just a handful of differential
# equations. Neural mass models are a powerful tool for studying large-scale brain dynamics and are widely
# used in computational psychiatry and neuroscience.
#
# We will use the **Jansen-Rit neural mass model** [2] to construct a model of the cortico-basal
# ganglia-thalamocortical (CBGTC) loop, based on Liu et al. (2020) [1], and explore how changes in circuit
# parameters can produce the β-band oscillations characteristic of Parkinson's disease.
