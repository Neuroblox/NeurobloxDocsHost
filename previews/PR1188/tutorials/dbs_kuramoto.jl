...
# - Use `create_adjacency_edges!` to specify all-to-all coupling.
...
# Now we specify the all-to-all coupling using `create_adjacency_edges!`. The weight matrix has
...
create_adjacency_edges!(g, K_matrix)
