...
# > **_NOTE_:** Because all connection weights in this network are uniform (every E→I pair has the same weight, every I→I pair has the same weight, etc.), we could alternatively pass the full connectivity as a weight matrix to `create_edges!` in one step. The explicit loops above are used here so you can see exactly which connections are being created. In a network with heterogeneous weights, the loop approach is the natural one. See the Resting State tutorial for an example of wiring from a weight matrix.
